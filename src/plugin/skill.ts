...
音声で知らせるときは \`PushNotification\` ツールを使う。この環境では hook が受けて本文をそのまま音声 (say) と webui の通知に流す。組み込みの説明にある制限はここでは効かない: 文字数の上限は無い、使う場面の限定も無い、人が在席かどうかは考慮しなくてよい (PC の前に居てもバックグラウンドで気づかないことがあるので、音声が要ると判断した時に使う)。
