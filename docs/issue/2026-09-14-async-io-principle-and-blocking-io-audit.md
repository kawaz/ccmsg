...
**メモリと CPU だけで完了し、入力量に比例しない処理が同期で、それ以外 (ファイル、ネットワーク、プロセス、エージェント応答、承認、lock など外部の完了を待つもの全て) はプロトコルも実装も非同期にする。**

単一スレッドなので「CPU で塞ぐな」とだけ言っても手段が無い。長い CPU 処理は IO の続きとして現れる — 読み込んだ transcript の行走査、全 record の parse / classify、各 record への正規表現の適用。IO から始まり入力量に比例する走査は record 単位や数千行単位で `await` を挟んで譲りながら処理し、1 回の呼び出しが分割できないもの (ユーザ指定の正規表現の適用など) は worker / 子プロセスに出す (issue `session-search-regex-unbounded`)。
