...
訂正 (kawaz 2026-09-14): 後半は「同期でよいのは入力量に比例しないもの」と言い直す。単一スレッドなので「CPU で塞ぐな」だけでは手段が無く何も変わらず、実際に長い CPU 処理は全部 IO の続き (読んだ transcript の行走査、全 record の parse / classify、正規表現の適用) である。IO から始まり入力量に比例する走査は record 単位や数千行単位で `await` を挟んで譲り、1 回の呼び出しが分割できないものは worker / 子プロセスに出す。DR-0015 §2 はこの形で書いてある。
