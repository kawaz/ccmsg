...
watch が報告できないものが 1 つだけある: **watch を張る瞬間と競合した変化**である (実測で 1 % 未満、落ちるのを観測したのはこれのみ)。よって購読が開いた少し後にもう 1 度読む — 周期ではなく購読 1 回につき 1 回の読みである。ディレクトリに張った watch がもう 1 つ見られないのは**そのディレクトリ自身の存在**で、無いディレクトリに watch は張れず、watch 中のディレクトリが消えたことは報告されない。よってそれぞれ**すぐ上の実在するディレクトリにも watch を張る**: ホスト最初の端末がディレクトリを作ったこと・それが消えたことを言うのはこれである。
