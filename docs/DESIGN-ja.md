...
**人の認証は passkey が担う** (DR-0001、実装の接続点は §3.3)。人の WS 接続は subprotocol `ccmsg.token.<access token>` を提示し、token が record に引けなければ handshake を断る。`source_ips` は「どこから来られるか」の allowlist として残り、**「誰が来たか」に答えるのは token** である。「何がそれを持っているか」は handshake が別に問う: `Origin` を token の family が持つ webui の origin と照合し、別の page を名乗るもの — および何も名乗らないもの — は upgrade が成立しない形で断る (契約 DR-0029)。page の一覧は設定に持たない (登録がそれを置く)。token を持たない handshake は匿名の人として通すのではなく断る。access token の `exp` が接続の期限で、`hello` の応答の `auth_expires_at` がそれを名乗り、同じ接続上の `auth.extend` で延ばす。UDS は到達すること自体がディレクトリの権限を通ることなので何も提示せず、期限も付かない。mesh は相手の TLS と `iss` / `aud` + proof (§7.2)、webhook は `Authorization: Bearer` で、どちらも経路自身が秘密を持つ。
...
**登録はローカルからしか始まらない。** `ccmsg daemon passkey add <unit> [endpoint] [--webui <URL>]` が登録用 URL (`<webui>#register=<jwt>`) と **6 桁のコード**を 1 組出す。人を送る先は webui で、登録が戻る先の instance は claims の中の endpoint — webui はどこにでも配れるので、2 つは別の値である (契約 DR-0029)。`--webui` がそれを名指し、省略すれば endpoint 自身 (自分で webui を配る通常形)。URL にコードは入らず、コードは端末にしか出ない — 2 つが別経路で browser に届くので、URL が漏れただけでは登録にならない。jwt を署名する secret は発行 instance のメモリにだけ在り、再起動で消える (永続鍵を持たない)。この 3 つの命令 (`add` / `list` / `remove`) は**契約の op ではなく instance の UDS にだけ届く管理フレーム**である: 契約はネットワーク越しに instance へ届くものの定義であり、登録はまさにそこへ出してはならないものだから。到達すること自体が権限である UDS は、監督者の制御要求と同じ足場になる。

**HTTP の 4 経路** (`/auth/challenge` `/auth/register` `/auth/assert` `/auth/refresh`) は「`needs_hello: false` の op を HTTP で運ぶもの」で、`request_id` は carrier が合成する。末尾照合なのは `ws` と同じ理由である (§3.2)。未認証で叩けるので 4 経路で 1 つの rate limit を共有する。CORS は **request の `Origin` が、この instance が知る webui の origin (ここに登録された credential の `webui`、および自分が発行してまだ持っている登録 URL の `webui`) のどれかと完全一致**する時だけ `Access-Control-Allow-Origin` + `Allow-Credentials` を echo し、外れていれば 403 を返す (契約 DR-0029)。設定には何も持たない。後半が、まだ credential の無い新しい webui での最初の登録に答えられる唯一の理由で、その知識は発行者の手元にしか無く複製もされない — だから登録は発行された場所でしか完了しない。**rp_id (ドメイン) では判定しない**: rp_id の配下を許すと兄弟サブドメインが `/auth/refresh` を `credentials: "include"` で叩けてしまい、cookie はドメイン単位で付くので本人の access token を読まれる。

**identity を決める 3 op は、page の JS が書けない 2 つのヘッダに照らされる**: claims または record が名指す webui の origin と一致する `Origin` と、page 自身の fetch が行き先の site に対して取りうる 3 つの関係 (`same-origin` / `same-site` / `cross-site`) のどれかを名乗る `Sec-Fetch-Site`。通る値の方を挙げるので、`none` (initiator が無い要求 = アドレスを打って開いた等) も仕様外の値も同じく断る。どちらのヘッダも不在は免除ではなく不一致で、断る時にどの検査で落ちたかは述べない (契約 DR-0029 / DR-0028)。`auth.challenge` は対象外 — 照らす相手がまだ無い段階の op で、配る challenge は発行者でしか 3 op に対して使い切れない — が、CORS の許可集合には他と同じく従う。

**credential は 2 つに束ねる: 入ってよい endpoint と、使ってよい webui** である (契約 DR-0029)。登録は claims の両方を record に書き、register / assert の受理条件は **`clientDataJSON.origin` == record の `webui` の `originOf`**、**`rpIdHash` == その `rpIdOf` の SHA-256**、**request が届いた先が endpoint 自身**である (scheme + authority は request から読むのではなくこの instance の公開アドレスで比べ、path はその base URL の下)。base URL に縛ることが隣の instance への入口を塞ぐ: record は mesh 全体に複製され、その中の endpoint が登録先を名乗る一方、page も ceremony も relying party も最初の instance のものなので、他のゲートは何も気づかない。`https://h/` と `https://h/personal/` は別 endpoint で別登録になる。origin も relying party も URL の隣には持たず、毎回そこから導く — 1 つの事実の写しが 2 つあると、人が実際に送られる URL と食い違いうるからである。webui を持たない古い record は名指す page が無いので無効で、移行は持たない (登録し直す。手順は `docs/runbooks/passkeys-webui-binding.md`)。

**token は署名しない opaque 値**で、検証は record の lookup である。**access token は family に 1 本で、その人が開いている複数のページ (タブ) が共有する**: rotate は cookie の refresh を毎回回すが、access は残り寿命が TTL の半分を切るまで据え置き、それ以降だけ mint し直す。毎回差し替えると、あるタブの読み込みが他のタブの持つ token を無効にしてしまう (半分は、ページが新しい値に気づくための猶予を寿命の半分残せる、最大の閾値)。access は応答の body、refresh は httpOnly cookie (`__Secure-ccmsg-<sha256(instance id + 改行 + sub) の先頭 16 hex>`、`HttpOnly; Secure; Path=<request のパスの /auth/ までの prefix>`)。site をまたぐかは credential ごとに決まる: webui が endpoint と同じ site なら `SameSite=Strict`、別の site なら `SameSite=None; Partitioned` — endpoint が持たない site から browser がそれを送る唯一の形で、1 つの site で得た session が別の site に持ち越されないようにする (契約 DR-0028)。cookie を分割しない browser はこの契約を話す環境ではないので、2 つ目の綴りは持たない。site は scheme と registrable domain で、cookie を送るか決める browser が読むのと同じ public suffix list から読む (private セクションも含める。その suffix の下の 2 host は browser にとって別 site だから)。registrable domain を持たない host (address literal、`localhost`) は host 自身に落とす。family を書けるのは mint した instance (`iss`) だけで、別の instance に届いた rotate は `auth.rotate` で `iss` へ転送する (§7.3 の経路)。**peer から届いた「自分が mint した family」の写しは受理しない** — 単一 writer なのだから、戻ってくる写しは必ず古い状態であり、失効させた family を復活させてしまう。直前 1 世代は再送の猶予として「前回の答え」を返し、それ以外の**退役済みの値の提示は世代を問わず family ごと失効させる** (提示された instance がその family の `iss` でなければ `auth.rotate` で `iss` に投げ、`iss` 側で失効させる。単一 writer は崩さない。`iss` が不達なら断るだけ)。失効は record の削除ではなく **family tombstone (7 日)** で、分断中の peer が持っていた生きた写しが復帰時に新しい書き込みとして戻ってこないようにする。退役の記録は退役した refresh の sha256 を family の `retired` に、その値本来の exp まで持つ。書けるのは `iss` だけだが複製はされるので、instance を再起動しても、別の instance に提示されても検知できる。exp を過ぎたものは次の rotate で落とす (その時点以降は、値自身の期限が断るものしか断らない)。失効と tombstone はどちらも、その sub の認証済み WS を閉じる — peer から届いた tombstone でも同じく閉じる。

`hello` の `auth_expires_at` は接続の期限で、`auth.extend` は **同じ利用者の** access token でしか延ばせない。carrier が http の op (`auth.challenge` / `auth.register` / `auth.assert` / `auth.token.refresh`) は **frame としては受けない**: cookie の読み書きは開いた接続の上ではできず、答えの片方が欠けたまま返すことになるので、dispatch が属性表を見て断る。carrier 側は handler に渡す前に `OP_SCHEMAS` を通し、3 op のどちらのヘッダも無い POST は断る。
...
- credential が登録先の endpoint と webui でだけ通る (別 origin / 別パス prefix の instance、別 origin の page のいずれでも拒否、§3.3)
