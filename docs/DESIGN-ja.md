...
**identity を決める 3 op は、page の JS が書けない 2 つのヘッダに照らされる**: claims または record が名指す webui の origin と一致する `Origin` と、`none` でない `Sec-Fetch-Site`。どちらも不在は免除ではなく不一致で、断る時にどの検査で落ちたかは述べない (契約 DR-0029 / DR-0028)。`auth.challenge` は対象外 — 照らす相手がまだ無い段階の op で、配る challenge は発行者でしか 3 op に対して使い切れない — が、CORS の許可集合には他と同じく従う。
