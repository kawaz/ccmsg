...
**credential は 2 つに束ねる: 入ってよい endpoint と、使ってよい webui** である (契約 DR-0029)。登録は claims の両方を record に書き、register / assert の受理条件は **`clientDataJSON.origin` == record の `webui` の `originOf`**、**`rpIdHash` == その `rpIdOf` の SHA-256**、**request が届いたパスが endpoint のパス**である。`https://h/` と `https://h/personal/` は別 endpoint で別登録になる。origin も relying party も URL の隣には持たず、毎回そこから導く — 1 つの事実の写しが 2 つあると、人が実際に送られる URL と食い違いうるからである。webui を持たない古い record は名指す page が無いので無効で、移行は持たない (登録し直す。手順は `docs/runbooks/passkeys-webui-binding.md`)。
...
- credential が登録先の endpoint と webui でだけ通る (別パス prefix の instance、別 origin の page では拒否、§3.3)
