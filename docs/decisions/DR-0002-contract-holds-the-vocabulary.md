...
この分界を名指しで決めるのは、v1 の daemon で測った偏りを繰り返さないためでもある: role の比較が handler の中に 95 箇所、同じ情報に 3 本の経路、8 種のタイマーのうち根拠が書かれていたのは 2 つだけ、そして**認可境界を持つ 3 モジュールはテストから 1 度も import されていなかった**。
...
- §9.2 は「境界を持つ経路にはその経路を直接呼ぶテストを置く」— 上の 95 箇所と同じ由来で、e2e で覆われているからと省かない
