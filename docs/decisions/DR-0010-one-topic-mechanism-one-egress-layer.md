...
| 値 | 決めること | 理由 |
|---|---|---|
| `FLUSH_PERIOD_MS` = 100ms | 1 つの終端に frame が出る頻度の上限 | 読み手にとって: 表示の再描画より近い frame は誰にも見えず、待ちは 1/4 秒あたりから遅延として読まれ始める。mesh にとって: relay された frame は hop ごとに 1 度待つので遅延は 100ms × hop 数。2 hop でも人が即時と読む範囲に収まる |
