...
**Settings are TypeScript where they are decisions and JSON where they are a list, and what is
edited is not what is read.** A person edits `${XDG_CONFIG_HOME:-~/.config}/ccmsg/`:
...
An instance and the supervisor read `$CCMSG_STATE_DIR/config/` instead: `satisfied.json`, the
value everything was read and checked into, beside a copy of each file it came from. **Nothing
reaches there that did not check out**, and nothing is evaluated twice — what runs is the value
that was checked, not a second reading of the files that produced it.

**The mesh is data.** `endpoints.json` is the same list on every host, and an instance finds
itself in it by its own id: that row is its endpoint, which is what a peer dials, what a
handshake carries as `iss` and `aud`, and what a person is handed to open a page at (§7.1). An
instance the list does not name has no address and is refused. A settings function is handed the
list and may read it — an instance that wants to know who else there is has it — but one that
hands back a different list is refused, because the mesh is not a settings function's to state.
Another host's instances are rows nobody here starts: `supervisor.json` is what this host runs,
and an id in it must be a row of the mesh and have settings of its own.

**An id is what a thing is, a name is what it is called.** An id is sixteen random bytes as hex,
generated by `daemon add` and written into the instance's state directory — the same one
everything the instance issued is keyed by (§3.6), so re-adding a config home that was removed
answers to the id it always had. The name is a label, defaults to the id, and lives inside the
file: renaming moves no file and rewrites no record.

**A start and a reload are the same one pass.** Everything is read; JSON is checked as JSON and
TypeScript is called (and awaited, since a settings function may answer with a promise); each
file is checked as far as it can be on its own, so that a mistake is reported where it was made.
The last gate is the whole: an id the supervisor starts must be an entry of the mesh and have a
file, two entries must not share an address, two instances must not share a port or a config
home, and no settings function may have restated the mesh. Only then is the state directory
written and the instance started or reloaded.

**A config that does not check out changes nothing.** The state directory is left alone, what is
wrong is written to the log and answered by `ccmsg daemon status`, and the host starts from what
was applied last — an instance that is serving a session is not something a typo in a file
should take away. The one exception is the first run, where there is nothing to fall back to.
`daemon add` and `daemon remove` write the edited files and then go through this same pass.

**`ccmsg config` is how the two sides are compared.** `list` says what each file is, what is
wrong with it, and whether it differs from the copy that was applied; `diff [file]` shows that
difference, and `diff --satisfied` shows what applying the edits would change about the whole;
`show` evaluates the edited files and prints the `satisfied.json` they would produce, writing
nothing; `revert <file> | --all` puts the applied copy back, keeping what it overwrote under
`$CCMSG_STATE_DIR/config.rejected/<path>.<time>` and printing where. Because `show` and
`diff --satisfied` call the settings functions to answer a question, **a settings function is
expected to have no side effects**.

Only the files named above are copied. What a settings file imports is its own business and is
not backed up here: a config home that loses one of those still starts, because what starts an
instance is the value and not the file.

`config_v2.ts` is handed `builtin`, the built-in defaults; an instance's file is handed `builtin`
and `default`, what `config_v2.ts` returned. Both are deeply frozen, and `config` is a mutable
copy of the level above, so a file edits what it was given and returns it. An instance's file
states `config.dir`, the absolute config home it answers for. **There is no merge rule, because
nothing merges**: a file is handed the whole of what it builds on and returns the whole of what
it runs with, so `config.dump.presets = […]` replaces and `.push(…)` adds, and the file says
which it meant.
...
- `ccmsg daemon supervise` — the foreground supervisor. It reads the config home's
  ...
  **The supervisor is the only route by which an instance is started.**
  ...
  `ccmsg daemon add <dir>` issues an instance id, writes
  `instances/instance-<id>.ts` — the label is the directory's own, the harness is read off it,
  and the port is the next free one after what this host has already handed out — puts the id and
  its loopback address in `endpoints.json` and the id in `supervisor.json`, and then goes through
  the pass of §8.2 before telling the supervisor. A proxy in front of the instance is a
  deployment fact nothing here can see, so an operator who has one edits that row.
  `remove <name | id | dir>` takes the id out of both files and deletes the settings, and leaves
  the state directory — the id there is what everything the instance issued is keyed by. `remove`
  stops it being looked after and **does not stop the child**: editing a list is not a shutdown,
  and a session already talking to that instance keeps talking to it.
...
- `ccmsg config list | diff | show | revert` — what is written against what is applied (§8.2).
  Its own command rather than one under `daemon` because what it is about is the files, which
  belong to the host rather than to any one instance: which of them is wrong, what an edit would
  change, and putting back the copy that checked out.
